...
//    print_map(bookStatuses);
    ...
    int sockfd = initialize_udp_socket(SERVER_H_UDP_PORT); // Port 0 lets the OS choose the port
...
int initialize_udp_socket(int port) {
    ...
    cout << "Server H is up and running using UDP on port " << port << "." << endl;
    return sockfd;
}
...
int receive_udp_commands(int sockfd, unordered_map<string, int> &bookStatuses) {
    ...
    while (true) {
        ...
        if (len > 0) {
            ...
            cout << "Server H finished sending the availability status of code " << bookCode
